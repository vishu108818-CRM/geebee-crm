  ...
  return (
    <main>
      ...
      <section className="content">
        ...
        {section === "Backorders" && <BackordersPanel orders={orders.filter((order) => order.status === "Advance order")} clients={clients} edit={(order) => show("order", order)} remove={(ids) => { setOrders((current) => current.filter((order) => !ids.includes(order.id))); logActivity("Removed advance order", "Backorders", ids.join(", ")); flash(`${ids.length} advance order${ids.length === 1 ? "" : "s"} removed`); }} />}{" "}
        {section === "Invoices" && (
          <Invoices
...
function BackordersPanel({ orders, clients, edit, remove }: { orders: Order[]; clients: Client[]; edit: (order: Order) => void; remove: (ids: string[]) => void }) {
  return <section className="panel record-panel"><div className="panel-head"><div><span className="overline">BACKORDER & ADVANCE ORDER</span><h2>Advance order register</h2><p>Only quantities unavailable in free stock appear here. Each advance order is editable and has its own PDF.</p></div></div><OrderTable orders={orders} edit={edit} remove={remove} sendWhatsApp={(order) => { const client = clients.find((item) => item.name === order.client); openWhatsApp(client?.whatsapp || client?.phone || "", `Dear ${order.client}, your GeeBee advance order #${order.id} is pending future dispatch. Advance value: ${money(orderTotal(order))}.`); }} /></section>;
}
