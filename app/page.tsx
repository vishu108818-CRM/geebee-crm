...
function BackordersPanel({ orders }: { orders: Order[] }) {
  return <section className="panel backorders-panel"><div className="panel-head"><div><span className="overline">BACKORDER & ADVANCE ORDER</span><h2>Future fulfilment</h2><p>Short quantities are reserved, given an Advance Order and advance invoice, and tracked until future dispatch.</p></div></div>{orders.map((order) => { const advanceOrder: Order = { ...order, id: `ADV-${order.id}`, product: order.backorders?.[0]?.product || "", sku: order.backorders?.[0]?.sku || "", quantity: order.backorders?.[0]?.quantity || 0, unitPrice: order.backorders?.[0]?.unitPrice || 0, products: order.backorders || [], status: "Advance order", payment: "Partial" }; return <article className="backorder-card" key={order.id}><div><b>{order.id}</b><span>{order.client}</span></div><div className="backorder-flow"><span>Available stock<br/><b>Dispatch now</b></span><i>→</i><span>Short quantity<br/><b>Backorder</b></span><i>→</i><span>{advanceOrder.id}<br/><b>Advance order</b></span><i>→</i><span>Reserved<br/><b>Future dispatch</b></span></div><div className="backorder-lines">{order.backorders?.map((line) => <p key={line.sku}><b>{line.sku}</b> {line.product} <strong>{line.quantity.toLocaleString("en-IN")} units</strong></p>)}</div><button type="button" className="outline" onClick={() => downloadOrderPdf(advanceOrder)}>Download Advance Order PDF</button></article>; })}{!orders.length && <div className="empty">No active backorders. New orders will be split automatically when ordered quantity exceeds free stock.</div>}</section>;
}
