...
const downloadOrderPdf = async (order: Order) => { const { jsPDF } = await import("jspdf"); const pdf = new jsPDF(); const logo = await logoData(); pdf.addImage(logo, "PNG", 14, 8, 42, 42); pdf.setTextColor(104, 29, 43); pdf.setFontSize(22); pdf.text("PURCHASE ORDER", 196, 20, { align: "right" }); pdf.setTextColor(60, 50, 54); pdf.setFontSize(10); pdf.text(`PO No: ${order.id}`, 196, 28, { align: "right" }); pdf.text(`Customer: ${order.client}`, 14, 60); pdf.text(`City: ${order.city || "—"}`, 14, 67); let y = 82; pdf.setFontSize(10); pdf.text("Product", 14, y); pdf.text("SKU", 105, y); pdf.text("Qty", 135, y); pdf.text("Rate", 155, y); pdf.text("Amount", 180, y); y += 5; pdf.line(14, y, 196, y); y += 7; (order.products || [{ product: order.product, sku: order.sku, quantity: order.quantity, unitPrice: order.unitPrice }]).forEach((line) => { pdf.text(line.product.slice(0, 36), 14, y); pdf.text(line.sku, 105, y); pdf.text(String(line.quantity), 135, y); pdf.text(String(line.unitPrice), 155, y); pdf.text(String(Math.round(line.quantity * line.unitPrice)), 180, y); y += 7; }); pdf.line(125, y, 196, y); y += 9; pdf.setFontSize(12); pdf.setTextColor(104, 29, 43); pdf.text(`Order Total: ${money(orderTotal(order))}`, 135, y); pdf.setFontSize(9); pdf.setTextColor(60, 50, 54); pdf.text(`Expected arrival: ${order.eta || "To be confirmed"}`, 14, y + 14); pdf.text("GeeBee — The Party Factory", 14, 285); pdf.save(`PO-${order.id}.pdf`); };
